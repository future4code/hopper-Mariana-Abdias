...
app.get('/afazeres', (req:Request, res:Response) => {
  return res.send(afazeres)
})

// Concluído
app.get("/afazeres/concluidos", (req:Request, res:Response) => {
  const completed = afazeres.filter((afazer) => {
...
// Em andamento
app.get("/afazeres/andamento", (req:Request, res:Response) => {
  const completed = afazeres.filter((afazer) => {
...
app.post("/afazeres",(req:Request,res:Response)=>{
  const newAfazer = req.body

  afazeres.push(newAfazer)
  return res.status(201).send(newAfazer)
})

// Exercício 6
app.put('/afazeres/:afazerId', (req:Request,res:Response) => {
  const afazerId = req.params.afazerId
  const updatedAfazer = req.body

  afazeres = afazeres.map(afazer => {
    
    if (Number(afazerId) === afazer.id) {
      return updatedAfazer
    }
  
    return afazer
  })
  return res.status(201).send(updatedAfazer)
})

// Exercício 7
app.delete('/afazeres/:afazerId', (req:Request, res:Response) => {
  const afazerId = req.params.afazerId

  afazeres = afazeres.filter(afazer => afazer.id !== Number(afazerId))
...
// Exercício 9
// Documentação criada:
// https://documenter.getpostman.com/view/21027375/VUqoQJMF
